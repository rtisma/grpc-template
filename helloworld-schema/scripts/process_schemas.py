...
class ProtobufGenerator(object):
    ...
    def _createSchemaFiles(self, destPath, schemasPath):
        ...
        overtureSchemasPath = os.path.join(overturePath, 'schemas/helloworld')
        if not os.path.exists(overtureSchemasPath):
            os.mkdir(overtureSchemasPath)
        overtureSchemashelloworldPath = os.path.join(overturePath, 'overture/schemas/helloworld')
        # if not os.path.exists(overtureSchemashelloworldPath):
        #     os.mkdir(overtureSchemashelloworldPath)

        # overtureSchemashelloworldPath = os.path.join(overtureSchemasPath, 'helloworld')
        # if not os.path.exists(overtureSchemashelloworldPath):
            # os.mkdir(overtureSchemashelloworldPath)
    ...
    def _doLineReplacements(self, line):
        ...
        """
        # ga4gh packages
        packageString = 'package org.icgc.dcc.helloworld.schema;'
        if packageString in line:
            return line.replace(
                packageString,
                'package overture.schemas.helloworld;')
        importString = 'import "overture/'
        if importString in line:
            return line.replace(
                importString,
                'import "overture/schemas/helloworld/')
        # google packages
...
def main(args=None):
    ...
    # defaultDestPath = "../python/overture/schemas/helloworld"
    defaultSchemasPath = 'src/main/proto/org/icgc/dcc/helloworld/schema'
    parser = argparse.ArgumentParser(
        description="Script to process Overture helloworld Protocol buffer schemas")
    parser.add_argument(
